...
class TeamService {
  ...
  async joinTeam(teamCode: string): Promise<Team | null> {
    ...
    if (res.status === 500) {
      throw new Error("An internal server occurred joining you to the team!");
    }
  ...
  async getTeamMembers(teamCode: string): Promise<TeamMember[]> {
    ...
    if (res.status === 500) {
      throw new Error("An internal server occured fetching the team's members");
    }
  ...
  async getTeamOverview(teamCode: string): Promise<TeamOverview | null> {
    ...
    if (res.status === 500) {
      throw new Error("An internal server occurred fetching the team overview");
    }
